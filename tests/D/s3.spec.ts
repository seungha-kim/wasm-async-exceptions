...
test('D/S3: suspended Promise rejects — observed console sequence', async ({ page }) => {
  const lines: string[] = [];
  page.on('console', (msg) => {
  ...
  await page.goto('http://localhost:8080/examples/D/s3/build/index.html');
  ...
  await Promise.race([
    ...
    page.waitForTimeout(5_000).then(() => lines.push('[timeout] no s3-done')),
  ]);

  expect(lines.length).toBeGreaterThan(0);
  expect(lines.join('\n')).toMatchSnapshot('D-S3-console.txt');
});
