...
test('E/S4: coroutine catch then re-await — observed console sequence', async ({ page }) => {
  const lines: string[] = [];
  page.on('console', (msg) => {
  ...
  await page.goto('http://localhost:8080/examples/E/s4/build/index.html');
  ...
  expect(lines.join('\n')).toMatchSnapshot('E-S4-console.txt');
});
