...
test('E/S2: coroutine await then throw — observed console sequence', async ({ page }) => {
  const lines: string[] = [];
  page.on('console', (msg) => {
  ...
  await page.goto('http://localhost:8080/examples/E/s2/build/index.html');
  ...
  expect(lines.join('\n')).toMatchSnapshot('E-S2-console.txt');
});
