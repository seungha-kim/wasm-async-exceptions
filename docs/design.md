...
두 Wasm 표준화 proposal은 위 에뮬레이션 일부를 Wasm 안으로 끌어들여 런타임 계층의
충돌 표면을 줄인다.
...
- 결과적으로 Asyncify/JS EH 에뮬레이션이 만들던 표면 일부는 줄어든다. 다만 JS Promise
  rejection을 C++ 예외 의미론으로 해석하는 경계는 자동으로 사라지지 않으며, Phase 3의
  target D 관측처럼 개발자가 명시적으로 설계해야 한다.
...
| **B** | Asyncify | Wasm EH | `-sASYNCIFY -fwasm-exceptions` | EH만 표준으로 바꾼 중간 경로 — exception-state stress 실패, normal-yield stress 통과 |
| **C** | JSPI | JS exception emulation | `-sASYNCIFY=2`(JSPI) | 코루틴만 표준으로 바꿨을 때 |
| **D** | JSPI | Wasm EH | `-sASYNCIFY=2 -fwasm-exceptions` | 두 런타임 표준 모두, JS rejection 경계는 별도 |
...
추가 stress 시나리오 S5-S17은 A/B/D에만 둔다. 이들은 JS Promise rejection을 쓰지
않고 모든 controlled Promise를 resolve하여, C++에서 시작한 exception과 suspend의
상호작용만 관찰한다.

- **S5 — catch 내부 suspend**: resolve 후 C++에서 throw → catch 진입 → catch 블록에서
  다시 await.
- **S6 — unwind 중 destructor suspend**: resolve 후 C++에서 throw → stack unwind 중
  destructor가 await.
- **S7 — catch 후 suspend/rethrow**: inner catch에서 await 후 `throw;`로 outer catch 도달
  여부 관찰.
- **S8 — multi-yield call chain 후 inner throw**: `level1 -> level2 -> level3`
  호출 체인의 각 단계에서 await한 뒤 가장 안쪽 C++ throw가 바깥 catch까지 도달하는지
  관찰.
- **S9 — catch가 호출한 helper 안 suspend**: catch 상태가 outer frame에 살아 있고,
  더 안쪽 helper에서 await할 때도 B가 실패하는지 관찰.
- **S10 — destructor가 호출한 helper 안 suspend**: unwind 중 destructor 자체가 아니라
  destructor가 호출한 helper에서 await해도 B가 실패하는지 관찰.
- **S11 — nested exception_ptr 저장 후 suspend/rethrow_if_nested**: nested exception
  state도 S12처럼 suspend 경계를 건너면 B가 실패하는지 관찰.
- **S12 — exception_ptr 저장 후 suspend/rethrow_exception**: catch를 빠져나온 뒤
  저장된 `std::exception_ptr`가 suspend를 건너 `std::rethrow_exception`될 수 있는지
  관찰.
- **S13 — copied exception object 후 suspend**: exception runtime state가 아니라
  복사된 C++ object payload만 건너면 어떤지 관찰.
- **S14 — 반복 normal suspend 후 throw**: live/captured exception state 없이 normal
  suspend를 여러 번 반복해도 B가 통과하는지 확인하는 대조군.
- **S15 — stable what pointer 후 suspend**: dangling pointer가 아닌 안정적인
  `what()` payload만 건너는 경우를 관찰.
- **S16 — copied what string 후 suspend**: `std::string` payload만 건너는 경우를 관찰.
- **S17 — resume 이후 exception_ptr 생성/소비**: `exception_ptr` 자체가 아니라
  captured state가 suspend를 건너는지가 핵심인지 확인하는 대조군.
...
- `CMakeLists.txt` — 24개 target/scenario 예제 빌드 matrix의 source of truth.
...
3. **Phase 2 — 부분 개선 (target B, C)**: 한 쪽만 표준으로 바꿨을 때 어디까지,
   ...
   구조적 불완전함이 남는다"는 점을 부연. 이후 S5-S17 stress에서
   B(Asyncify+Wasm EH)는 JS rejection 없이도 실패하여, EH만 먼저 표준으로 바꾸는
   경로가 안전한 중간 단계가 아님을 별도 데이터로 기록한다.
4. **Phase 3 — 표준 조합 검증 (target D)**: 런타임 표준 둘 다 켰을 때 (JSPI+Wasm EH)
...
6. **Phase 3.5 — 지표 보강**: correctness 관측이 끝난 뒤, 비용과 표면 증거를
   ...
   반복 가능한 수집 명령을 우선한다.
7. **Phase 4 — resolution-only stress**: A/B/D에 S5-S17을 추가해 JS Promise
   rejection을 배제한 C++ exception/suspend stress를 관측한다. 관측 결과
   S5-S7/S9-S12는 A/D가 통과하고 B는 `null function`/`unreachable`로 실패한다.
   S8/S14/S17은 A/B/D 모두 통과하여, B의 실패 조건이 반복/깊은 normal yield나
   `exception_ptr` 자체가 아니라 live 또는 captured Wasm EH exception state가 suspend
   경계를 건너는 패턴임을 좁혀준다. S13/S15/S16은 B가 `PASS:done`까지 도달한 뒤
   post-done `unreachable`을 내는 경고 케이스로 기록한다.
8. **Phase 5 — 발표 산출물**: `README.md` 데모 링크, 아주 간단한 글/슬라이드 노트.
...
- **Asyncify + Wasm EH 중간 경로의 취약성**: Emscripten은 `ASYNCIFY=1`과
  `-fwasm-exceptions` 혼용에 대해 경고한다. S5-S7/S9-S12 관측상 이 조합은 JS
  rejection 없이도 C++ exception/suspend stress에서 실패한다. S8/S14/S17 관측상 단순
...
- target B, C에서 “한 쪽만 표준으로 바꿨을 때 어디까지 개선되고 어디서 다시 막히는지”를
  관측하고, 잔존 한계를 기록한다.
- A/B/D의 S5-S17 stress를 통해 JS rejection을 제거한 상태에서도 Asyncify+Wasm EH의
  failure surface가 있는지 기록한다.
