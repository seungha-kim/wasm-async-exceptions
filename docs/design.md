...
추가 stress 시나리오 S5-S10/S12/S14는 A/B/D에만 둔다. 이들은 JS Promise rejection을 쓰지
...
3. **Phase 2 — 부분 개선 (target B, C)**: 한 쪽만 표준으로 바꿨을 때 어디까지,
   ...
   구조적 불완전함이 남는다"는 점을 부연. 이후 S5-S7/S9/S10/S12 stress에서
...
6. **Phase 3.5 — 지표 보강**: correctness 관측이 끝난 뒤, 비용과 표면 증거를
   ...
   반복 가능한 수집 명령을 우선한다.
7. **Phase 4 — resolution-only stress**: A/B/D에 S5-S10/S12/S14를 추가해 JS Promise
   rejection을 배제한 C++ exception/suspend stress를 관측한다. 관측 결과
   S5-S7/S9/S10/S12는 A/D가 통과하고 B는 `null function`/`unreachable`로 실패한다.
   S8/S14는 A/B/D 모두 통과하여, B의 실패 조건이 반복/깊은 normal yield가 아니라
   live 또는 captured Wasm EH exception state가 suspend 경계를 건너는 패턴임을
   좁혀준다.
...
- **Asyncify + Wasm EH 중간 경로의 취약성**: Emscripten은 `ASYNCIFY=1`과
  ...
  multi-yield 후 throw는 통과하므로, 마이그레이션 경로로 다룰 때 "live/captured
  exception state가 suspend를 건너는 패턴"을 별도 위험으로 문서화한다.
...
- target B, C에서 “한 쪽만 표준으로 바꿨을 때 어디까지 개선되고 어디서 다시 막히는지”를
  관측하고, 잔존 한계를 기록한다.
- A/B/D의 S5-S10/S12/S14 stress를 통해 JS rejection을 제거한 상태에서도 Asyncify+Wasm EH의
