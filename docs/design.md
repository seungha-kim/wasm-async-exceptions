...
| **B** | Asyncify | Wasm EH | `-sASYNCIFY -fwasm-exceptions` | EH만 표준으로 바꾼 중간 경로 — S5-S7 stress에서 실패, S8은 통과 |
...
추가 stress 시나리오 S5-S8은 A/B/D에만 둔다. 이들은 JS Promise rejection을 쓰지
...
3. **Phase 2 — 부분 개선 (target B, C)**: 한 쪽만 표준으로 바꿨을 때 어디까지,
   ...
   구조적 불완전함이 남는다"는 점을 부연. 이후 S5-S7 stress에서 B(Asyncify+Wasm EH)는
   JS rejection 없이도 실패하여, EH만 먼저 표준으로 바꾸는 경로가 안전한 중간 단계가
   아님을 별도 데이터로 기록한다.
...
6. **Phase 3.5 — 지표 보강**: correctness 관측이 끝난 뒤, 비용과 표면 증거를
   ...
   반복 가능한 수집 명령을 우선한다.
7. **Phase 4 — resolution-only stress**: A/B/D에 S5-S8을 추가해 JS Promise rejection을
   배제한 C++ exception/suspend stress를 관측한다. 관측 결과 S5-S7은 A/D가 통과하고
   B는 `null function`/`unreachable`로 실패한다. S8은 A/B/D 모두 통과하여, B의 실패
   조건이 단순한 multi-yield 후 throw가 아니라 catch/unwind/rethrow 상태가 살아 있는
   suspend임을 좁혀준다.
...
- **Asyncify + Wasm EH 중간 경로의 취약성**: Emscripten은 `ASYNCIFY=1`과
  `-fwasm-exceptions` 혼용에 대해 경고한다. S5-S7 관측상 이 조합은 JS rejection 없이도
  C++ exception/suspend stress에서 실패한다. S8 관측상 단순 multi-yield 후 inner throw는
  통과하므로, 마이그레이션 경로로 다룰 때 "예외 상태가 살아 있는 suspend"를 별도 위험으로
  문서화한다.
...
- target B, C에서 “한 쪽만 표준으로 바꿨을 때 어디까지 개선되고 어디서 다시 막히는지”를
  관측하고, 잔존 한계를 기록한다.
- A/B/D의 S5-S8 stress를 통해 JS rejection을 제거한 상태에서도 Asyncify+Wasm EH의
