...
> 실사용은 에뮬레이션 경로를 쓸 수밖에 없다(JSPI/Wasm EH의 정확한 지원 현황은
> Phase 0에서 caniuse/MDN 기준으로 다시 점검). 그래서 이 프로젝트는 “현재의 페인”과
> “미래의 구원”을 나란히 보여준다.
...
### 2.3 예제 명명 규칙
`examples/<target>/<scenario>/`, 예: `examples/A/s2/`. 각 디렉토리 구성:
- `main.cpp`: 최소한의 재현 코드
- `build.sh`: 빌드 커맨드
- `run.sh`: 브라우저/Node 자동 진입 (`npm run serve` 등)
...
1. **Correctness**: catch 도달 여부, 멈춤(hang), throw가 잡히지 않고 JS까지
   새어나가는지(`uncaught`), Asyncify `unreachable` 트랩.
...
├── src/                      # 여러 예제가 공유하는 헬퍼(throw/await 유틸, fetch/IO)
├── examples/
│   └── <target>/<scenario>/
└── scripts/
    └── toolchain.sh          # emsdk 버전 핀, JSPI 브라우저 점검 스크립트
```
...
1. **Phase 0 — 툴체인 & 베이스라인** (`docs/background.md`, `scripts/toolchain.sh`): emsdk
   버전 고정, JSPI/Wasm EH가 켜진 브라우저 확인 스크립트, S1 베이스라인 빌드.
...
- **JSPI/Wasm EH 미지원 브라우저**: Phase 3는 nightly/불특정 브라우저만 가능.
  target D 예제는 “실행 가능 브라우저 명시” 조건을 두고, 불가능 시 Phase 4 노트에
  “미지원 환경에서는 시뮬레이션으로 재현”로 빠둔다.
