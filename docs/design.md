...
| **B** | Asyncify | Wasm EH | `-sASYNCIFY -fwasm-exceptions` | EH만 표준으로 바꾼 중간 경로 — S5-S7 stress에서 실패 |
...
추가 stress 시나리오 S5-S7은 A/B/D에만 둔다. 이들은 JS Promise rejection을 쓰지
...
6. **Phase 3.5 — 지표 보강**: correctness 관측이 끝난 뒤, 비용과 표면 증거를
   ...
   반복 가능한 수집 명령을 우선한다.
7. **Phase 4 — resolution-only stress**: A/B/D에 S5-S7을 추가해 JS Promise rejection을
   배제한 C++ exception/suspend stress를 관측한다. 관측 결과 A/D는 통과하고 B는
   `null function`/`unreachable`로 실패한다.
...
- **Asyncify + Wasm EH 중간 경로의 취약성**: Emscripten은 `ASYNCIFY=1`과
  ...
  C++ exception/suspend stress에서 실패하므로, 마이그레이션 경로로 다룰 때 별도 위험으로
  문서화한다.
...
- target B, C에서 “한 쪽만 표준으로 바꿨을 때 어디까지 개선되고 어디서 다시 막히는지”를
  관측하고, 잔존 한계를 기록한다.
- A/B/D의 S5-S7 stress를 통해 JS rejection을 제거한 상태에서도 Asyncify+Wasm EH의
