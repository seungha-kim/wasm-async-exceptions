...
- **Asyncify + Wasm EH는 안전한 중간 마이그레이션 경로가 아니다.** JS rejection을
  쓰지 않는 S5-S7 stress에서도 B만 `null function`/`unreachable`로 실패했고,
  A(Asyncify+JS EH)와 D(JSPI+Wasm EH)는 통과했다.
...
| B | Asyncify | Wasm EH | EH만 표준으로 바꾸는 위험한 중간 경로 — S5-S7 실패 |
...
추가 stress 매트릭스 S5-S7은 A/B/D만 대상으로 한다. Promise는 모두 resolve하고
C++ throw/catch/unwind와 suspend만 섞는다. 관측 결과는 A/D 통과, B 실패다.
...
- [x] resolution-only C++ exception stress 관측 (Phase 4: S5-S7)
