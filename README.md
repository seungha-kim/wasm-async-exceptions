...
- **Asyncify + Wasm EH는 안전한 중간 마이그레이션 경로가 아니다.** JS rejection을
  쓰지 않는 S5-S7/S9/S10/S12 stress에서도 B만 `null function`/`unreachable`로
  실패했다. 다만 S8/S14는 B도 통과하여, 실패 조건은 "여러 yield 뒤 throw" 전반이
  아니라 live/captured Wasm EH exception state가 suspend 경계를 건너는 쪽으로
  좁혀진다. 이 pass/fail 패턴은 기본 핀인 emsdk 6.0.1뿐 아니라 emsdk 4.0.23
  전체 재빌드/재실행에서도 동일하게 재현됐다.
...
추가 stress 매트릭스 S5-S10/S12/S14는 A/B/D만 대상으로 한다. Promise는 모두
resolve하고 C++ throw/catch/unwind와 suspend만 섞는다. 관측 결과 S5-S7/S9/S10/S12는
A/D 통과, B 실패이고, S8/S14는 A/B/D 모두 통과다.

S8/S14가 B에서도 통과한 이유는 suspend 시점이 다르기 때문이다. 이들의 suspend는
예외가 발생하기 전 정상 call chain 안에서 끝나고, 그 뒤 C++ `throw`는 동기적으로
outer catch까지 unwind된다. 반대로 S5-S7/S9/S10은 catch handler, destructor unwind,
rethrow 준비 상태처럼 **live C++ exception state**가 있는 동안 suspend한다. S12는
catch가 끝난 뒤 suspend하지만, 저장해 둔 `std::exception_ptr`를 resume 후 다시
활성화한다. 따라서 B의 실패 조건은 "여러 yield 뒤 throw"가 아니라 "Wasm EH의 live
또는 captured exception state를 Asyncify가 suspend 경계 너머로 보존해야 하는 패턴"으로
좁혀진다.
...
- [x] resolution-only C++ exception stress 관측 (Phase 4: S5-S10/S12/S14)
