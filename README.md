...
- **Asyncify + Wasm EH는 안전한 중간 마이그레이션 경로가 아니다.** JS rejection을
  쓰지 않는 S5-S7/S9-S12 stress에서도 B만 `null function`/`unreachable`로
  실패했다. 다만 S8/S14/S17은 B도 통과하여, 실패 조건은 "여러 yield 뒤 throw"
  전반이 아니라 live/captured Wasm EH exception state가 suspend 경계를 건너는 쪽으로
  좁혀진다. S13/S15/S16은 payload read와 `PASS:done`까지 도달하지만 B에서 종료 직후
  `unreachable` pageerror가 남는 별도 경고 케이스다. 이 pass/fail 패턴은 기본 핀인
  emsdk 6.0.1뿐 아니라 emsdk 4.0.23 전체 재빌드/재실행에서도 동일하게 재현됐다.
...
| B | Asyncify | Wasm EH | EH만 표준으로 바꾸는 위험한 중간 경로 — exception-state stress 실패, normal-yield stress 통과 |
...
추가 stress 매트릭스 S5-S17는 A/B/D만 대상으로 한다. Promise는 모두 resolve하고
C++ throw/catch/unwind와 suspend만 섞는다. 관측 결과 S5-S7/S9-S12는 A/D 통과,
B 실패이고, S8/S14/S17은 A/B/D 모두 통과다. S13/S15/S16은 A/D가 clean pass,
B가 `PASS:done` 이후 `unreachable` pageerror를 내는 중간 경고 케이스다.

S8/S14/S17이 B에서도 통과한 이유는 exception state가 suspend 경계를 건너지 않기
때문이다. S8/S14의 suspend는
예외가 발생하기 전 정상 call chain 안에서 끝나고, 그 뒤 C++ `throw`는 동기적으로
outer catch까지 unwind된다. S17은 `exception_ptr`를 resume 이후에 만들고 다음 suspend
전에 소비한다. 반대로 S5-S7/S9/S10은 catch handler, destructor unwind, rethrow 준비
상태처럼 **live C++ exception state**가 있는 동안 suspend한다. S11/S12는 catch가 끝난
뒤 suspend하지만, 저장해 둔 `std::exception_ptr`를 resume 후 다시 활성화한다. 따라서
B의 hard failure 조건은 "여러 yield 뒤 throw"가 아니라 "Wasm EH의 live 또는 captured
exception state를 Asyncify가 suspend 경계 너머로 보존해야 하는 패턴"으로 좁혀진다.
S13/S15/S16은 payload 자체는 읽히지만 post-done trap이 남는 별도 표면이다.
...
| S9 | A/B/D | catch 블록이 호출한 helper 체인 안에서 suspend해도 catch state가 보존되는가? |
| S10 | A/B/D | destructor가 호출한 helper 체인 안에서 suspend해도 unwind state가 보존되는가? |
| S11 | A/B/D | nested exception을 `exception_ptr`로 저장한 뒤 suspend 이후 nested rethrow가 가능한가? |
| S12 | A/B/D | `std::exception_ptr`로 저장한 예외를 suspend 이후 `rethrow_exception`할 수 있는가? |
| S13 | A/B/D | catch에서 복사한 concrete exception object는 suspend 이후에도 읽을 수 있는가? |
| S14 | A/B/D | 여러 번 반복된 normal resolve suspend 이후 C++ throw/catch가 계속 정상 동작하는가? |
| S15 | A/B/D | 안정적인 `what()` 포인터 payload만 suspend 경계를 건너면 어떻게 되는가? |
| S16 | A/B/D | `what()`을 복사한 `std::string` payload만 suspend 경계를 건너면 어떻게 되는가? |
| S17 | A/B/D | resume 이후 생성한 `exception_ptr`를 다음 suspend 전에 rethrow하면 안전한가? |
...
- [x] resolution-only C++ exception stress 관측 (Phase 4: S5-S17)
- [x] emsdk 4.0.23 교차 검증: `npm test` 59개 통과, 동일 pass/fail 패턴 재현
